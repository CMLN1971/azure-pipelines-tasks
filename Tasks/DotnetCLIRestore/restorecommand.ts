...
export async function run(): Promise<void> {
    ...
    try {
        ...
        let projectFiles = CreateProjectFiles(packageName, packageVersion);

        try {
            for (const projectFile of projectFiles) {
                await dotNetRestoreAsync(dotnetPath, projectFile, packagesDirectory, packageVersion, configFile, noCache, verbosity);
            }
        } finally {
            credCleanup();
...
function CreateProjectFiles(packageName: string, version: string): string[] {
    let projectFilePath: string = getProjectFilePath();
...
function dotNetRestoreAsync(dotnetPath: string, projectFile: string, packagesDirectory: string, version: string, configFile: string, noCache: boolean, verbosity: string): Q.Promise<number> {
    ...
    if (packagesDirectory) {
        dotnet.arg('package');
        dotnet.arg('OctocatApp');
    }

    if (packagesDirectory) {
        dotnet.arg('--package-directory');
